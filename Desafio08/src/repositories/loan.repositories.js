...
function findActiveLoanByBookIdRepository(bookId) {
    return new Promise((resolve, reject) => {
        const sql = `SELECT * FROM loans WHERE bookId = ?`;
...
async function deleteLoanRepository(loanId) {
    return new Promise((resolve, reject) => {
        // Primeiro verifica se o empréstimo existe
        findLoanByIdRepository(loanId)
            .then(loan => {
                if (!loan) {
                    logger.error(`Error deleting loan: Loan with ID ${loanId} not found`);
                ...
                db.run(sql, [loanId], function(err) {
                    ...
                    resolve(loan); // retorna o empréstimo deletado
                });
            })
            .catch(err => {
...
function updateLoanRepository(updateLoan, loanId) {
    return new Promise((resolve, reject) => {
        ...
        const sql = `UPDATE loans SET userId = ?, bookId = ?, dueDate = ? WHERE id = ?`;
